!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!     restart I/O fuctions for SION backend
...
!       local variables
  integer         :: bytes, count, rc, datasize
  ...
  if ( restart_handle%if_acc_size ) then
    ...
    call fsion_fwrite( var, bytes, count, restart_handle%sid, rc )
...
!       local variables
  integer :: bytes, count, rc, datasize
    ...
    call fsion_fread( var, bytes, count, restart_handle%sid, rc )
    if ( count /= rc ) then 
      ierr = -1
    else
      ierr = 0
