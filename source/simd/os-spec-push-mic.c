...
Relativistic particle pusher, Intel MIC optimized version
...
// This version uses 23 bit approximations for 1/x and 1/sqrt(x) instead of the
...
\
  vrgamma = _mm512_rsqrt23_ps( _mm512_add_ps( vutsq, c1 ) );\
\
  vene = _mm512_mul_ps( vutsq, \
         _mm512_rcp23_ps( _mm512_add_ps( _mm512_rcp23_ps(vrgamma), c1) )) ;\
\
...
{\
  __m512 const c1 = _mm512_set1_ps( 1.0f );\
  register __m512 vut1, vut2, vut3, vutsq;\
  register __m512 vgamma, vtem_gamma, votsq;\
\
  ve1 = _mm512_mul_ps( ve1, vtem );\
  ve2 = _mm512_mul_ps( ve2, vtem );\
  ve3 = _mm512_mul_ps( ve3, vtem );\
...
// This version uses 23 bit approximations for 1/sqrt(x) instead of the
// full precision division and square root

#define VRGAMMA( vrg, vu1, vu2, vu3 ) {                            		  \
   const __m512 c1 = _mm512_set1_ps( 1.0f );                       		  \
   (vrg) = _mm512_rsqrt23_ps( _mm512_fmadd_ps( (vu3), (vu3),              \
...
*/  
  ...
  for ( k = 0; k < np_total; k += p_cache_size_1D ) {
    ...
    for( i = 0; i < np; i+=VEC_WIDTH ) {
      ...
      vx0  = _mm512_load_ps( (float const*) x );
      vix0 = _mm512_load_epi32( (__m512i const* ) ix );
      ...
      // Load momenta
      vu1 = _mm512_load_ps( (float const*) ( u      ) );
      vu2 = _mm512_load_ps( (float const*) ( u + 16 ) );
      vu3 = _mm512_load_ps( (float const*) ( u + 32 ) );
      ...
      {
        register __m512 vrg;
        register __m512 vtr1;
        register __m512i vitr1;
        ...
        vq = _mm512_load_ps( (float const*) q );
        ...
        vv2 = _mm512_mul_ps( vu2, vrg ); // this is required for current deposition
        vv3 = _mm512_mul_ps( vu3, vrg ); // this is required for current deposition

        // Store momenta
        _MM512_TRANSPOSE3v16_PS( vu1, vu2, vu3 );
        _mm512_store_ps( u      , vu1 );
        _mm512_store_ps( u + 16 , vu2 );
        _mm512_store_ps( u + 32 , vu3 );
        ...
        _mm512_store_ps( (__m512 *) x, vx1 );

        // find cell crossings and store
        vitr1 = _mm512_cvtfxpnt_round_adjustps_epi32( vtr1, _MM_ROUND_MODE_NEAREST, _MM_EXPADJ_NONE ) ;
        _mm512_store_epi32( (__m512i *) dix, vitr1 );

        // Trim cell indexes and store
        vix0 = _mm512_add_epi32( vix0, vitr1 ); 
        _mm512_store_epi32( (__m512i *) ix, vix0 );
  ...
  for ( k = 0; k < np_total; k += p_cache_size_2D ) {
    ...
    np = ( np_total - k < p_cache_size_2D ) ? np_total - k : p_cache_size_2D;
      ...
      // ---------- advance position and get velocities
      {
        ...
        // find cell crossings and store
        vitr1 = _mm512_cvtfxpnt_round_adjustps_epi32( vtr1, _MM_ROUND_MODE_NEAREST, _MM_EXPADJ_NONE ) ;
        vitr2 = _mm512_cvtfxpnt_round_adjustps_epi32( vtr2, _MM_ROUND_MODE_NEAREST, _MM_EXPADJ_NONE ) ;
  ...
  double *dx, double *dt )
{
   ...
   fprintf(stderr, "(*error*) MIC SIMD version of 2D cylindrical algorithm"
                   " not yet implemented" );
   abort();
}
  ...
  for ( k = 0; k < np_total; k += p_cache_size_3D ) {
    ...
    np = ( np_total - k < p_cache_size_3D ) ? np_total - k : p_cache_size_3D;
      ...
      // ---------- advance position and get velocities
      {
        ...
        // find cell crossings and store
        vitrx = _mm512_cvtfxpnt_round_adjustps_epi32( vtrx, _MM_ROUND_MODE_NEAREST, _MM_EXPADJ_NONE ) ;
        vitry = _mm512_cvtfxpnt_round_adjustps_epi32( vtry, _MM_ROUND_MODE_NEAREST, _MM_EXPADJ_NONE ) ;
        vitrz = _mm512_cvtfxpnt_round_adjustps_epi32( vtrz, _MM_ROUND_MODE_NEAREST, _MM_EXPADJ_NONE ) ;
