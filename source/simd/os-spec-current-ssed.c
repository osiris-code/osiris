...
inline void vwl_s2( __m128d const vqn, __m128d const vx0, __m128d const vx1, __m128d vwl[] ) {
  ...
  __m128d d    = _mm_sub_pd( vx1, vx0 );
  __m128d s1_2 = _mm_sub_pd( c1_2, vx0 );
  __m128d p1_2 = _mm_add_pd( c1_2, vx0 );
  
  __m128d n = _mm_mul_pd( vqn, d );
  
  // qn * d * ( s1_2 - 0.5 * d ) 
  vwl[0] = _mm_mul_pd( n, _mm_sub_pd( s1_2, _mm_mul_pd( c1_2, d ) ) );
  // qn * d * ( p1_2 + 0.5 * d ) 
  vwl[1] = _mm_mul_pd( n, _mm_add_pd( p1_2, _mm_mul_pd( c1_2, d ) ) );  
...
inline void vwl_s3( __m128d const vqn, __m128d const vx0, __m128d const vx1, __m128d vwl[] ) {
   ...
   __m128d const c1_2 = _mm_set1_pd( 0.5 );
   __m128d const c3_4 = _mm_set1_pd( 0.75 );
   __m128d const c1_3 = _mm_set1_pd( 1.0/3.0 );
   
   __m128d d = _mm_sub_pd( vx1, vx0 );
   __m128d s = _mm_sub_pd( c1_2, vx0 );
   __m128d p = _mm_add_pd( c1_2, vx0 );
   __m128d d_3 = _mm_mul_pd( c1_3, d );

   vwl[0] = _mm_mul_pd( c1_2, _mm_sub_pd( _mm_mul_pd( s, s ),       _mm_mul_pd( d, _mm_sub_pd( s, d_3 ) ) ) );
   vwl[1] = _mm_sub_pd( _mm_sub_pd( c3_4, _mm_mul_pd( vx0, vx0 ) ), _mm_mul_pd( d, _mm_add_pd( vx0, d_3 ) ) );
   vwl[2] = _mm_mul_pd( c1_2, _mm_add_pd( _mm_mul_pd( p, p ),       _mm_mul_pd( d, _mm_add_pd( p, d_3 ) ) ) );

   __m128d n = _mm_mul_pd( vqn, d );
   vwl[0] = _mm_mul_pd( n, vwl[0] );
   vwl[1] = _mm_mul_pd( n, vwl[1] );
   vwl[2] = _mm_mul_pd( n, vwl[2] );
...
inline void vwl_s4( __m128d const vqn, __m128d const vx0, __m128d const vx1, __m128d vwl[] ) {
   ...
   __m128d const c1_2 = _mm_set1_pd( 0.5 );
   __m128d const c1_4 = _mm_set1_pd( 0.25 );
   __m128d const c1_6 = _mm_set1_pd( 1.0/6.0 );
   __m128d const c3_2 = _mm_set1_pd( 1.5 );
   __m128d const c3_4 = _mm_set1_pd( 0.75 );
   __m128d const c1_3 = _mm_set1_pd( 1.0/3.0 );
   ...
   __m128d t = _mm_sub_pd( vx0, c1_6 );
   __m128d u = _mm_add_pd( vx0, c1_6 );

   __m128d s2 = _mm_mul_pd( s, s );
   __m128d s3 = _mm_mul_pd( s2, s );

   __m128d p2 = _mm_mul_pd( p, p );
   __m128d p3 = _mm_mul_pd( p2, p );

   __m128d d_2 = _mm_mul_pd( d, c1_2 );
   __m128d d_4 = _mm_mul_pd( d, c1_4 );
   

   vwl[0] = _mm_mul_pd( c1_6, _mm_add_pd( s3, _mm_mul_pd( d, _mm_sub_pd( _mm_mul_pd( d, _mm_sub_pd( s, d_4 ) ),  _mm_mul_pd( c3_2, s2 ) ) ) ) );
   vwl[1] = _mm_add_pd( _mm_add_pd( _mm_sub_pd( c2_3, p2 ), _mm_mul_pd( c1_2, p3 ) ), _mm_mul_pd( d, _mm_add_pd( _mm_sub_pd( _mm_mul_pd( c3_4, _mm_mul_pd( t, t ) ), c1_3 ), _mm_mul_pd( d_2, _mm_add_pd( t, d_4 ) ) ) ) );
   vwl[2] = _mm_sub_pd( _mm_add_pd( _mm_sub_pd( c2_3, s2 ), _mm_mul_pd( c1_2, s3 ) ), _mm_mul_pd( d, _mm_add_pd( _mm_sub_pd( _mm_mul_pd( c3_4, _mm_mul_pd( u, u ) ), c1_3 ), _mm_mul_pd( d_2, _mm_add_pd( u, d_4 ) ) ) ) );
   vwl[3] = _mm_mul_pd( c1_6, _mm_add_pd( p3, _mm_mul_pd( d, _mm_add_pd( _mm_mul_pd( d, _mm_add_pd( p, d_4 ) ),  _mm_mul_pd( c3_2, p2 ) ) ) ) );

   __m128d n = _mm_mul_pd( vqn, d );
   vwl[0] = _mm_mul_pd( n, vwl[0] );
   vwl[1] = _mm_mul_pd( n, vwl[1] );
   vwl[2] = _mm_mul_pd( n, vwl[2] );
   vwl[3] = _mm_mul_pd( n, vwl[3] );
...
Note: In 2D there was a division by 2 removed from the calculation of the perpendicular
      weights, so jnorm must be dx / dt / 2. In 3D it was a division by 3, so 
      jnorm must be dx / dt / 3. To avoid repeating this every time the 
...
(double * const current, int const * const size, int const * const offset, 
 double * const norm, t_split_buf1D * const part) {
  ...
  __m128d vx0, vx1, vq, vvy, vvz;
  
  DECLARE_ALIGNED_16( int idx[VEC_WIDTH] );

      
  __m128d vqnx, vqvz, vqvy;
  __m128d vs0x[NP], vs1x[NP]; 
  ...
  for( i = 0; i < np; i += VEC_WIDTH ) {
    ...
    // get j2,j3 current
    for ( k1 = 0; k1 < NP; k1++ ) {
      __m128d vwp1 = _mm_mul_pd( c1_2, _mm_add_pd(vs0x[k1], vs1x[k1]) );
      j2[k1].v2 = _mm_mul_pd( vqvy, vwp1 );
      j3[k1].v2 = _mm_mul_pd( vqvz, vwp1 );
    }
 ...
 double * const norm, t_split_buf2D * const part)
{
  ...
  // Normally this would be VEC_WIDTH but since we are using __m128i values to store the
  // positions we have 2 extra (unused) values
  ...
  for( i = 0; i < np; i+=VEC_WIDTH ) {
    ...
    // load 2 particles
    LOAD2P2D( part, i, vx0, vx1, vy0, vy1, vq, vvz, vix, viy );
    ...
    for( k2 = 0; k2 < NP; k2++ ) {
      for ( k1 = 0; k1 < NP; k1++ ) {
		...
		j3[k1][k2].v2 = _mm_mul_pd( vqvz, _mm_add_pd( tmp1, _mm_mul_pd( tmp2, c1_2 ) ) );
      }
    }
                       ...
                       double * const norm, t_split_buf3D * const part)
{
  ...
  for( i = 0; i < np; i += VEC_WIDTH ) {
	...
	// load 2 particles
    LOAD2P3D( part, i, vx0, vx1, vy0, vy1, vz0, vz1, vq, vix, viy, viz );
    ...
    for( k2 = 0; k2 < NP; k2++ ) {
      for( k1 = 0; k1 < NP; k1++ ) {
		 __m128d tmp1, tmp2;        
		 const __m128d c1_2 = _mm_set1_pd(0.5);
         ...
         vwp1[k2][k1].v2 = _mm_add_pd( tmp1, _mm_mul_pd( tmp2, c1_2));
         ...
         vwp2[k2][k1].v2 = _mm_add_pd( tmp1, _mm_mul_pd( tmp2, c1_2));

         tmp1 =  _mm_add_pd( _mm_mul_pd( vs0x[k1], vs0y[k2]), _mm_mul_pd( vs1x[k1], vs1y[k2]));
         tmp2 =  _mm_add_pd( _mm_mul_pd( vs0x[k1], vs1y[k2]), _mm_mul_pd( vs1x[k1], vs0y[k2]));
         vwp3[k2][k1].v2 = _mm_add_pd( tmp1, _mm_mul_pd( tmp2, c1_2));
