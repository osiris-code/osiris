...
{                                                                                             \
   ...
   vutsq = _mm256_add_pd( _mm256_mul_pd( vut3, vut3 ),\
                          _mm256_add_pd( _mm256_mul_pd( vut2, vut2 ),\
                                         _mm256_mul_pd( vut1, vut1 ) ) );\
   \
   ...
   \
   vu1 = _mm256_add_pd( _mm256_mul_pd(vb3, vut2), vut1 );\
   vu2 = _mm256_add_pd( _mm256_mul_pd(vb1, vut3), vut2 );\
   vu3 = _mm256_add_pd( _mm256_mul_pd(vb2, vut1), vut3 );\
   \
   vu1 = _mm256_sub_pd( vu1, _mm256_mul_pd(vb2, vut3)  );\
   vu2 = _mm256_sub_pd( vu2, _mm256_mul_pd(vb3, vut1)  );\
   vu3 = _mm256_sub_pd( vu3, _mm256_mul_pd(vb1, vut2)  );\
   \
   /* Perform second half of the rotation. */\
  votsq = _mm256_add_pd( _mm256_mul_pd(vb1, vb1),    c1 );\
  votsq = _mm256_add_pd( _mm256_mul_pd(vb2, vb2), votsq );\
  votsq = _mm256_add_pd( _mm256_mul_pd(vb3, vb3), votsq );\
  \
  votsq = _mm256_div_pd( _mm256_set1_pd( 2.0 ), votsq ) ;\
   ...
   \
  vut1 = _mm256_add_pd( _mm256_mul_pd(vb3, vu2), vut1 );\
  vut2 = _mm256_add_pd( _mm256_mul_pd(vb1, vu3), vut2 );\
  vut3 = _mm256_add_pd( _mm256_mul_pd(vb2, vu1), vut3 );\
  \
  vut1 = _mm256_sub_pd( vut1, _mm256_mul_pd(vb2, vu3) );\
  vut2 = _mm256_sub_pd( vut2, _mm256_mul_pd(vb3, vu1) );\
  vut3 = _mm256_sub_pd( vut3, _mm256_mul_pd(vb1, vu2) );\
  \
...
Note: Need to check use of rsqrt_pd
*****************************************************************************************/

#define VRGAMMA( vrg, vu1, vu2, vu3 ) {  \
  __m256d const c1 = _mm256_set1_pd( 1.0 );                                              \
  (vrg) = _mm256_add_pd( _mm256_mul_pd((vu3),(vu3)),                                     \
                         _mm256_add_pd(_mm256_mul_pd((vu2),(vu2)),                       \
                                       _mm256_add_pd(_mm256_mul_pd((vu1),(vu1)),c1 )));  \
  (vrg) = _mm256_div_pd( c1, _mm256_sqrt_pd(vrg) );                                      \
}
    ...
    for( i = 0; i < np; i += VEC_WIDTH ) {
      ...
      // Interpolate fields
      {
        ...
        // Interpolate E field
        {
          ...
          for ( k1 = 0; k1 < NP; k1 ++ ) {
            ve1 = _mm256_add_pd( ve1, _mm256_mul_pd( f1point[k1], vwxh[k1] ) );
            ve2 = _mm256_add_pd( ve2, _mm256_mul_pd( f2point[k1],  vwx[k1] ) );
            ve3 = _mm256_add_pd( ve3, _mm256_mul_pd( f3point[k1],  vwx[k1] ) );
          }
        ...
        // Interpolate B field
        {
          ...
          for ( k1 = 0; k1 < NP; k1 ++ ) {
            vb1 = _mm256_add_pd( vb1, _mm256_mul_pd( f1point[k1],  vwx[k1] ) );
            vb2 = _mm256_add_pd( vb2, _mm256_mul_pd( f2point[k1], vwxh[k1] ) );
            vb3 = _mm256_add_pd( vb3, _mm256_mul_pd( f3point[k1], vwxh[k1] ) );
          }
      ...
      // Load charge
        ...
        vene_group = _mm256_add_pd( _mm256_mul_pd( vq, vene ), vene_group );
        ...
        // Push positions
        __m256d vx1 = _mm256_add_pd( vx0, _mm256_mul_pd( vv1, vdt_dx1 ) );
  ...
  double *dx, double *dt, double *ene )
{
  ...
  for ( k = 0; k < np_total; k += p_cache_size_2D ) {
	...
	for( i = 0; i < np; i+=VEC_WIDTH ) {
	  ...
	  // ---------- advance momenta, and get time centered energy
	  _MM256_LOAD4v3_PD( vu1, vu2, vu3, u );
	  ...
	  // ---------- advance position and get velocities
	  {
		...
		// Accumulate energy
		vene_group = _mm256_add_pd( vene_group, _mm256_mul_pd( vq, vene ) );
	...
	// Deposit current from all virtual particles in the group
  ...
  for ( k = 0; k < np_total; k += p_cache_size_2D ) {
	...
	for( i = 0; i < np; i+=VEC_WIDTH ) {
	  ...
	  // ---------- advance position in cylindrical geometry
	  {
		...
		__m128i vitr1 = _mm256_cvttpd_epi32( vtr1 ) ;
		__m128i vitr2 = _mm256_cvttpd_epi32( vtr2 ) ;
		_mm_store_si128( (__m128i *) dix, vitr1 );
		_mm_store_si128( (__m128i *) diy, vitr2 );
  ...
  for ( k = 0; k < np_total; k += p_cache_size_3D ) {
	...
	for( i = 0; i < np; i += VEC_WIDTH ) {
	  ...
	  // ---------- advance position and get velocities
	  {
        ...
        // Push positions
		__m256d vx1 = _mm256_add_pd( vx0, _mm256_mul_pd( _mm256_mul_pd( vu1, vrg ), vdt_dx1 ) );
		__m256d vy1 = _mm256_add_pd( vy0, _mm256_mul_pd( _mm256_mul_pd( vu2, vrg ), vdt_dx2 ) );
		__m256d vz1 = _mm256_add_pd( vz0, _mm256_mul_pd( _mm256_mul_pd( vu3, vrg ), vdt_dx3 ) );
		...
		__m128i vitrx = _mm256_cvttpd_epi32( vtrx );
		__m128i vitry = _mm256_cvttpd_epi32( vtry );
		__m128i vitrz = _mm256_cvttpd_epi32( vtrz );
